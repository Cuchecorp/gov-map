...
| 56. CRON-AUDIT — Auditoría E2E de los 9 workflows de ingesta | v6.0 | 0/TBD | Not started | - |
| 57. CRON-FIX — Hardening dos-etapas + hash-check + crons verdes | v6.0 | 0/TBD | Not started | - |
...
## 🚧 v6.0 — Confiabilidad y comprensión (In Progress)

**Milestone Goal:** El dato llega solo y se entiende solo: toda la ingesta programada corre perfecta end-to-end (fuente→R2 crudo content-addressed, R2→Supabase, hash-check, idempotencia, monitoreo de frescura); gov-map estrena identidad visual propia (ícono serio, public-policy, no estilo IA); cada visualización se entiende sin explicación externa, validada por loop BrowserOS.
**Granularity:** fine
**Numbering:** continúa desde v5.0 — Phase 55 fue la última de v5.0; v6.0 arranca en **Phase 56** (no reset).

### Coverage

- v6.0 requirements: 12 (CRON 5, AUTOR 2, BRAND 2, COMP 3)
- Mapped to phases: 12/12 ✓
- Orphaned: 0

### Phases (v6.0)

- [ ] **Phase 56: CRON-AUDIT — Auditoría E2E de los 9 workflows de ingesta** - Inventario auditado de cada cron (veredicto + gap-list accionable); producto es el diagnóstico que alimenta la fase de hardening.
...
### Phase 56: CRON-AUDIT — Auditoría E2E de los 9 workflows de ingesta

**Goal:** El operador tiene un inventario auditado de los 9 workflows de GitHub Actions (agenda, leyes, lobby×2, probidad, fichas-backfill, backup-parlamentario, backfill, deploy) con veredicto por cron — corre / no corre / por qué — y una gap-list accionable que alimenta la fase de hardening (Phase 57). Sin este diagnóstico, el hardening opera a ciegas.
**Depends on:** Phase 55 (v5.0 complete)
**Requirements:** CRON-01
**Autonomy:** autónomo para inspección de código/YAML + inventario; verificación de corrida LIVE = checkpoint operador (secrets, billing GH).
**Success Criteria** (what must be TRUE):

  1. Existe un inventario documentado de los 9 workflows con veredicto por cron: corre en PROD / no corre / por qué (secrets faltantes, billing bloqueado, triggers mal configurados, etapa R2 ausente, hash-check ausente).
  2. La gap-list identifica con precisión de archivo+línea qué le falta a cada conector para cumplir DOS ETAPAS LOCKED y hash-check — el insumo directo de Phase 57.
  3. Se identifica el estado de billing GH Actions (bloqueado o desbloqueado) y se documenta cuáles workflows pueden correr hoy vs cuáles necesitan fallback local.
  4. El inventario es re-ejecutable como comprobación: el operador puede correr cada workflow en dispatch manual y confirmar el veredicto con logs reales.

**Plans:** TBD

### Phase 57: CRON-FIX — Hardening dos-etapas + hash-check + crons verdes

**Goal:** Cada conector recurrente cumple las DOS ETAPAS LOCKED re-ejecutables por separado (fuente→R2 crudo content-addressed, R2→Supabase desde el crudo) y hace hash-check/ETag antes de descargar — una corrida sin novedades sale temprano sin re-descarga ni escritura, verificable en logs. Los crons de novedades corren verdes en su scheduler (GitHub Actions L–V con secrets cargados) o, si billing GH sigue bloqueado, existe un fallback local documentado y probado.
**Depends on:** Phase 56 (gap-list auditada — saber qué arreglar)
**Requirements:** CRON-02, CRON-03, CRON-04
**Autonomy:** autónomo para código (añadir etapas R2, hash-check); encender LIVE + verificar corrida verde = checkpoint operador (secrets DEEPSEEK/R2/SUPABASE, billing GH).
**Success Criteria** (what must be TRUE):

  1. Cada conector recurrente tiene DOS ETAPAS claramente separadas y re-ejecutables independientemente: (a) fuente→R2 crudo con `PUT If-None-Match: *` (412 = ya existía = éxito idempotente); (b) R2→Supabase leyendo SOLO del crudo — re-ingestar a Supabase nunca vuelve a tocar la fuente.
  2. Todos los conectores del cron hacen hash-check o ETag ANTES de descargar: una corrida sin novedades sale temprano con log `[skip] sin novedades` sin re-descarga ni escritura a Supabase (verificable en los logs del workflow).
...
**Goal:** El operador puede consultar en un solo lugar la frescura por fuente (última corrida exitosa, último snapshot R2, último upsert a Supabase) y detectar staleness sin bucear logs — reporte CLI o superficie admin, con umbral de alerta configurable por fuente.
**Depends on:** Phase 57 (pipelines sanos — monitorear un pipeline roto no tiene valor; la frescura debe reflejar estado real)
**Requirements:** CRON-05
**Autonomy:** autónomo para construir el reporte/CLI; superficie admin (si la hay) es UI sin DDL sensible.
**Success Criteria** (what must be TRUE):

  1. Existe una herramienta (CLI `pnpm freshness` o superficie admin `/admin/freshness`) que lista, por fuente, la última corrida exitosa, el último snapshot R2, el último upsert a Supabase y cuántos días han pasado desde cada uno.
...
**Goal:** Los autores de cada proyecto del corpus quedan poblados (`proyecto.autores` hoy 0/136) mediante el pipeline R2→Supabase con reconciliación fail-closed (solo match determinista/confirmado enlaza a la maestra), desbloqueando la ficha de proyecto para mostrar autoría con guarda de identidad — F48 diferida de v5.0 por gap de datos, ahora resuelta.
**Depends on:** Phase 57 (pipeline dos-etapas hardeneado — la ingesta de autores debe cumplir el patrón LOCKED antes de escribir a Supabase)
**Requirements:** AUTOR-01, AUTOR-02
**Autonomy:** autónomo para código (parser de autores, reconciliación, writer, ficha); corrida LIVE con datos reales = checkpoint operador.
**Success Criteria** (what must be TRUE):

  1. El campo `proyecto.autores` (o tabla relacional `proyecto_autor`) queda poblado para el corpus actual: el pipeline lee del crudo en R2 (no de la fuente), reconcilia cada nombre de autor contra la maestra de parlamentarios con `matchDeterminista` fail-closed, y escribe solo matches `determinista`/`confirmado`; lo demás queda como mención cruda (`autor_crudo`).
  2. Una segunda corrida del mismo pipeline produce 0 upserts nuevos (idempotencia verificada en logs).
...
### Phase 60: BRAND — Ícono/identidad visual gov-map

**Goal:** gov-map estrena un ícono/logo propio — simple, serio, interesante, public-policy oriented — completamente integrado en PROD (favicon multi-resolución, imagen OG/social, header del sitio y manifest), coherente con el design system crema/petróleo existente. Explícitamente NO el estilo típico hecho-con-IA (wordmark con palabras en fuentes distintas). La selección opera entre ≥3 propuestas conceptuales distintas con decisión del operador.
**Depends on:** Phase 55 (v5.0 complete — design system cerrado y PROD estable). Independiente de Phases 56-59; puede ejecutarse en paralelo con 57/58/59 pero se ubica aquí para que BRAND esté integrado antes del barrido BrowserOS de Phase 61.
**Requirements:** BRAND-01, BRAND-02
**Autonomy:** propuestas generadas autónomamente (SVG código puro, sin herramienta externa); selección = checkpoint operador; integración técnica autónoma tras selección; deploy = checkpoint operador (Docker Linux + wrangler).
**Success Criteria** (what must be TRUE):

  1. El operador eligió un ícono entre ≥3 propuestas conceptuales distintas (SVGs generados como código puro); la selección queda registrada con razón.
  2. El SVG maestro tiene variantes: mono (un color), invertido (para fondos oscuros) y favicon (≥16×16 legible sin texto).
...
### Phase 61: COMP — Comprensión de visualizaciones (loop BrowserOS)

**Goal:** Toda visualización clave (cruces entre parlamentarios, ficha parlamentario, ficha proyecto, /red, charts de v5) se entiende a la primera sin conocimiento previo, validado por loop BrowserOS captura→lectura fría→corrección→re-captura. La sección de cruces estrena leyenda "cómo leer esto" explícitamente anti-causal; cada visualización lleva título orientado a la pregunta que responde, leyenda y fuente/fecha visible; todo hallazgo P0/P1 queda corregido y re-verificado.
**Depends on:** Phase 59 (AUTOR-02 — ficha de proyecto tiene nueva superficie de autoría que BrowserOS también revisa), Phase 60 (BRAND-02 — ícono integrado en PROD; BrowserOS revisará la identidad visual junto con la comprensión)
**Requirements:** COMP-01, COMP-02, COMP-03
**Autonomy:** autónomo para correcciones de comprensión (texto, leyenda, título); BrowserOS = herramienta del workflow; hallazgos P0/P1 con corrección autónoma; P2+ diferidos.
**Success Criteria** (what must be TRUE):

  1. La sección de cruces entre parlamentarios tiene una leyenda visible "Cómo leer esto" que explica: qué es una señal factual, cómo interpretar los conteos, y qué NO afirma el sistema (anti-causal explícito, sin vocabulario de correlación ni causalidad); una lectura fría en BrowserOS produce veredicto "comprensible sin contexto previo".
  2. Un barrido BrowserOS de las superficies clave (ficha parlamentario, ficha proyecto, /red, charts de v5) con loop captura→corrección→re-captura cierra todos los hallazgos P0/P1 de comprensión; la re-captura final muestra veredicto "mejorado" o "comprensible" en cada superficie corregida.
