...
- [x] 97-02-PLAN.md — Deploy real: build OpenNext (Docker) + wrangler deploy + secret publishable + config Supabase OTP; leer el log del middleware
...
- [x] 99-02-PLAN.md — migración 0066: RPC bounded actualidad_senales_panel (aguja completa) + alta en PUBLIC_RPC_ALLOWLIST
- [x] 99-03-PLAN.md — workspace @obs/actualidad: kmeans determinista seed-fija (label=mode(materia), no LLM) + run-actualidad-prod-cli
- [x] 99-04-PLAN.md — actualidad-refresh.yml (clon leyes-weekly sin R2, intradía L-V) + apply live-DB 0065/0066 (checkpoint operador, sin flip de régimen)
...
- [x] 101-02-PLAN.md — Wave 0 guards (SUPERFICIES_RELACIONES + allowlist) + bloque `<section id=relaciones>` above-the-fold en la ficha, componentes intactos (REL-02)
...
- [x] 102-01-PLAN.md — Wave 0: gate fail-closed + anti-flip + linter idioms + co_votacion∉/red + 0068 escrita + allowlist + .env.example (VSIM-02/03)
- [x] 102-02-PLAN.md — Aplicar 0068 a PROD + pgTAP contra schema aplicado (VSIM-01)
- [x] 102-03-PLAN.md — Componente neutral + 5ª sección gated en /comparar + RTL ON/OFF/M=0 + dossier legal pending (VSIM-01/02)
...
- [x] 103-02-PLAN.md — migraciones 0069/0070/0071 (suscripcion RLS to authenticated + notificacion_envio service_role-only + consentimiento) + pgTAP dos-usuarios A-no-ve-B (NOTIF-01/02/04)
