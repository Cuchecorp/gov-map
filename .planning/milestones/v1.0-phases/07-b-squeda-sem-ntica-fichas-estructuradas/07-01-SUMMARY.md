...
**Acción ejecutada:** las 11 migraciones (0001..0011) aplicadas en orden al proyecto nube `<PROJECT_REF_REDACTADO>` (región sa-east-1, vía pooler IPv4 `<POOLER_HOST_REDACTADO>`; el host directo es IPv6-only y la máquina no tiene IPv6). Tracking registrado en `supabase_migrations.schema_migrations`. El proyecto nube estaba vacío (0 tablas) → cutover limpio.
...
- Wiring de env de la app para nube: `SUPABASE_URL`=https://<SUPABASE_HOST_REDACTADO> + anon/publishable key de la nube (hoy el .env tiene `SUPABASE_API_URL` + `SUPABASE_SECRET_KEY`). `SUPABASE_DB_URL` (pooler) ya escrita en .env.
