...
- [x] **CRON-01**: El operador puede ver un inventario auditado de los 9 workflows de GitHub Actions (agenda, leyes, lobby×2, probidad, fichas-backfill, backup-parlamentario, backfill, deploy) con veredicto por cron: corre / no corre / por qué (secrets faltantes, billing, triggers), y una gap-list accionable.
...
- [x] **BRAND-01**: gov-map tiene un ícono/logo propio: SVG maestro + variantes (mono, invertido, favicon), simple, serio, interesante, public-policy oriented; explícitamente NO el estilo típico hecho-con-IA (wordmark con palabras en fuentes distintas). Selección entre ≥3 propuestas conceptuales distintas.
- [x] **BRAND-02**: El ícono queda integrado y verificado en PROD: favicon multi-resolución, imagen OG/social, header del sitio y manifest, coherente con el design system crema/petróleo existente.
...
- [x] **COMP-02**: Barrido BrowserOS de las superficies clave (ficha parlamentario, ficha proyecto, /red, charts de v5) con loop captura→corrección→re-captura; todo hallazgo de comprensión P0/P1 corregido y re-verificado con nueva captura.
...
| BRAND-01 | Phase 60 | Complete |
| BRAND-02 | Phase 60 | Complete |
| COMP-01 | Phase 61 | Complete |
| COMP-02 | Phase 61 | Complete |
...
---
*Archived 2026-07-09 at v6.0 completion. All 12 requirements satisfied (audit: tech_debt, 0 gaps). Outcomes: CRON-02 parcial-sistémico (solo leyes registra source_snapshot; lobby --from-r2 sin cuerpo) → backlog v6.1.*
