...
- [ ] **RED-01**: `/red` con seed muestra SOLO el ego-network real (el seed + sus vecinos directos, aristas entre ellos), NUNCA los ~136 nodos apiñados. Sin seed: estado inicial que orienta (explicación + selector prominente), jamás el grafo completo.
- [ ] **RED-02**: Layout legible y determinista que NO implique afinidad (decisión LOCKED de F18: nunca force-simulation): p.ej. radial ego-céntrico con vecinos en anillo en orden neutro (alfabético), etiquetas legibles sin zoom, tope de vecinos con "ver más" honesto, usable en móvil 390px. La leyenda "cómo leer este grafo" se actualiza al layout nuevo.
...
- [ ] **BUSQ-01**: Todo proyecto presente en la DB tiene ficha + embedding (hoy: 156 proyectos, 74 fichas, 74 embeddings, 60 ideas matrices): backfill LOCAL completa el gap, reintenta fallidos y registra causa por boletín; techo honesto documentado (PDF escaneado, fuente caída, etc. — nunca fabricar).
...
| REQ-ID | Phase | Status |
|--------|-------|--------|
| RED-01 | Phase 62 | Pending |
| RED-02 | Phase 62 | Pending |
