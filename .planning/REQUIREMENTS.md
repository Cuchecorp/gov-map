...
- [x] **PANEL-01**: Un ciudadano que llega a la portada ve, en cada tile del panel, los sujetos del hecho — boletín + título + fecha con verbo explícito — no contadores mudos. Vía Opción A: los 6 bloques de `materializar_senales()` pueblan `evidencia` jsonb `{"total": N, "items": [...]}` (migración 0080 aditiva; supresión conserva `'{}'`; cap por recencia PROHIBIDO sin total — anti-B-01).
- [x] **PANEL-02**: Cada ítem enlaza a su ancla de ficha (`/proyecto/{b}#estado|#timeline|#votaciones`) o a `/agenda#...?semana=`; ningún link de agenda produce 404 (guard de existencia en el `left join proyecto` del materializador: ítem siempre emitido con `en_corpus:false` — nunca inner-join, el conteo jamás diverge del detalle). Helper central de links internos (hoy no existe).
- [x] **PANEL-03**: El panel renderiza la propuesta editorial (spike `v13.0-editorial-portada.md`): sala-semana (L2), comisiones citadas con cruce urgencia↔citación (L1+L5+L6), urgencias por grado (L3, "95" muere, conteo por boletines), movimiento reciente nombrado, archivos/retiros (proyectos, no eventos: "2 eventos de 1 proyecto"), y el tile materia ELIMINADO. Decisiones O-1..O-7 ratificadas por el operador en el loop de diseño.
- [x] **PANEL-04**: Votaciones recientes (L4) visibles — VSIM ON verificado + sign-off legal dado 2026-07-30. Por votación (jamás agregada por boletín), Senado `resultado` NULL → "resultado no informado por la fuente"; conteo de votantes solo confirmados (283.550, jamás 549k).
- [x] **PANEL-05**: Semántica de fechas correcta en todo el panel: el hecho (pasado o futuro) vive en el cuerpo con verbo explícito (idioms aprobados `Citado el …`, `Urgencia … vigente desde …`, `En tabla de sala de la Cámara del …`); el footer lleva SOLO frescura `según fuente al …`; `"datos al"` desaparece; `fecha_captura` jamás visible.
- [x] **PANEL-06**: Grafía de cámara única en todo el panel — fix **4-15/D2** en el materializador (`0065:233,261`), no en el cliente.
- [x] **PANEL-07**: Cobertura y asimetrías declaradas: citaciones "23 Senado · 0 Cámara"; tabla de Cámara presentada como "tabla semanal" (fila sintética `camara:sesion:2026-W31`, numero/tipo/hora NULL — jamás fabricar "sesión N.º a las HH:MM"); ceros con denominador; vacío honesto con causa.
...
- [x] **PANEL-09**: Loop de diseño BrowserOS con Opus mirando el deploy hasta que quede bien; cierre por fragmento DOM + captura (baseline ya capturado en `spikes/assets/`); densidad validada a 390px; ningún criterio visual subjetivo. Cierra también **B-02** (el tile con denominador ausente muere con el tile materia) y **H-01** (re-deploy + verificación `/comparar`).

### NEWS — Crons de noticias vinculadas (objetivo 2)

- [x] **NEWS-01**: Conector RSS en dos etapas LOCKED: robots.txt consultado ANTES de cada host + rate-limit 2-3 s/host + UA identificatorio + RSS **crudo** content-addressed a R2 (`fuente/recurso/fecha/sha256.ext`, `If-None-Match: *`) → parseo SIEMPRE desde R2, jamás de la fuente. Hash-check antes de descargar. Cierra los 4 huecos de régimen de Is Chile Safe.
- [x] **NEWS-02**: Fuentes 100% RSS: **5 medios directos** (BioBioChile, Cooperativa, LaTercera, LaCuarta, Ex-Ante — todos con RSS y robots.txt verificados vivos); pre-filtro léxico legislativo determinista antes de gastar LLM. *(Enmendado 2026-08-05 por D-132-A: Google News RSS Search DESCARTADO — `news.google.com/robots.txt` prohíbe `/rss/` para todo UA, verificado con `robots-parser@3.0.1`; usarlo violaría el régimen LOCKED de robots.txt. Evidencia: `.planning/phases/132-*/132-RESEARCH.md`.)* *(Cláusula N, 2026-08-05: si el probe A4 de 132-01 retira un host por bloqueo de WAF al `Fetcher` de Node, NEWS-02 se cumple con **N ≥ 3** medios directos y la baja documentada — autorizado por D-132-A / premortem F-7; con N < 3 la fase PARA.)*
- [x] **NEWS-03**: Taxonomía legislativa definida y CONGELADA antes de medir; golden set propio con etiquetas revisadas ANTES de cualquier benchmark (lección Is Chile Safe: techo 65,9% era de labels); thresholds pre-registrados y congelados; el input crudo que el LLM vio se guarda re-runnable.
...
- [x] **DEBT-03** (**H-06**): La regla de selección del timeline queda gobernada por una query escrita (85 `Hito del` vs 99 eventos en `14309-04`), con su criterio declarado. Cerrado en 131-01/131-03 — ver `.planning/phases/131-debt-ficha-regla-del-timeline-co-autoria-sin-truncamiento/131-EVIDENCIA-APLICACION.md` (sección F: reproducido contra PROD tras el merge).
- [x] **DEBT-04** (**fila 3.3**): La co-autoría de `/comparar` emite membresía de par (RPC rediseñada, firma v2 paralela — precedente `0060`, jamás alterar la viva `42P13`), sin truncamiento silencioso a 20. Cerrado en 131-02/131-03 — ver `.planning/phases/131-debt-ficha-regla-del-timeline-co-autoria-sin-truncamiento/131-EVIDENCIA-APLICACION.md` (0083 aplicada a PROD, pgTAP 12/12, testigo A=B=92).
