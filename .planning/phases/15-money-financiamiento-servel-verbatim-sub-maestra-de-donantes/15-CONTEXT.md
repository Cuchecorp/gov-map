...
- **A1 RE-RESUELTO (decisión de operador 2026-06-19, principio de finalidad del dato): la fuente SERVEL NO trae RUT — ni donante ni candidato, solo NOMBRES.** El enlace candidato→parlamentario YA NO es RUT-exacto: usa el **pipeline de identidad confirmado/auditado** (`correrPipeline` de `@obs/adjudication` → `EnlaceConfirmado | null`, solo `determinista`/`confirmado`, con gate humano + `identidad_audit`), enlazando por NOMBRE del candidato (funcionario público). Espeja `packages/lobby/src/reconciliar-sujeto.ts` (Phase 11). Esto restaura la fiscalización con exactitud (homónimos adjudicados, NO string-match ingenuo). Ver [[finalidad-del-dato-linking-politicos]].
  - **Solo el NOMBRE del candidato (funcionario público) va al pipeline/LLM; PII de donantes NUNCA** (data-routing gate intacto). El RUT del donante (si apareciera) queda interno, deny-by-default, jamás llave de enlace.
  - Reemplaza la regla previa "RUT-exacto, nunca por nombre" para el linking de funcionarios. Estados honestos: `enlazado` (confirmado/auditado) puede poblarse de verdad; `no-verificado` (el pipeline no confirmó / quedó en cola de revisión humana); `no-ingestado`.
