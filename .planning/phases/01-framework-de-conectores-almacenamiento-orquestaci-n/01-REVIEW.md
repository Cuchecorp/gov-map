...
### CR-01: Edge Function handler is unauthenticated and ACKs caller-controlled msg_ids (data loss)
...
### CR-02: Rate-limiter state does not survive across Edge Function invocations — 2-3s policy not enforced
...
### CR-03: Fetcher has no origin allowlist (SSRF / abuse surface)
...
### CR-04: Service-role key is persisted in queryable pg_net tables
...
### WR-01: rate-limit/robots host can be spoofed independently of the fetched URL
...
### WR-02: Daily-cache gate is racy — concurrent runs both miss and double-fetch
...
### WR-03: robots/fetch throws on malformed URL; robots fail-open masks real errors
...
### WR-04: processBatch swallows all errors with no logging or classification
...
### WR-05: BACKFILL_ITERATIONS is unbounded and unvalidated
...
### WR-06: Connector run is not wrapped in an ingest_run lifecycle; ctx is a fabricated cast
...
### WR-07: pg_cron sub-minute schedule string may be silently rejected
...
### IN-01: drift `lastFingerprint` ordered by `fetched_at` is tie-ambiguous
...
### IN-02: structuralPaths treats single-element and N-element arrays identically but empty arrays specially
...
### IN-03: env var names diverge between worker and migration comments
...
### IN-04: R2 `ext` and `resource`/`source` are interpolated into the key without sanitization
...
### IN-05: `max_requests` parameter is dead (only `perform`ed)
