 ...
 *
 * `TERMINOS_LINK_EXT`, `TERMINOS_COBERTURA` y `TERMINOS_PROHIBIDOS` se movieron a
 * `./terminos-insinuacion.ts` (D-133-J2, vía B acotada) — este archivo los IMPORTA,
 * no los re-tipea, para que `packages/news/src/eval/taxonomia-guard.test.ts` (G1)
 * pueda leerlos por disco sin invertir la dirección de dependencia del monorepo.
 * `NEGACIONES_LOCKED` (abajo) NO se movió: importa constantes de componentes `.tsx`.
 */
...
/**
 * G3 (D-133-J3, T-133-04): lee una superficie declarada del allowlist y LANZA si no
 * existe, en vez de saltarla en silencio. Un allowlist con una ruta muerta es un guard
 * ciego que sale verde — el falso verde que este helper existe para cerrar. Sustituye
 * los DOS bloques de lectura-con-salto-silencioso que vivían en los tests (1) y (1b).
 */
function leerSuperficie(rel: string): string {
  try {
    return readFileSync(path.join(APP_ROOT, rel), "utf-8");
  } catch {
    throw new Error(
      `Superficie declarada inexistente: ${rel}. Un allowlist con una ruta muerta ` +
        `es un guard ciego que sale verde. Si el archivo se movió, corrige la ruta; ` +
        `si se borró, quita la entrada.`,
    );
  }
}
...
describe("(1) Guard — ninguna superficie de voto ni MONEY insinúa (texto renderizado)", () => {
  it("piso: TERMINOS_PROHIBIDOS conserva >= 90 términos tras la extracción", () => {
    // D-133-J2/T-133-05: la extracción a terminos-insinuacion.ts perdió términos si el
    // conteo cae por debajo del piso medido (92, con el mismo método de stripTsComments
    // que G1 usa). Un `it.each` no sirve aquí (cero-vacuo con array vacío) — es un
    // conteo directo sobre el array importado.
    expect(
      TERMINOS_PROHIBIDOS.length,
  ...
  it("(1g) G3 no-hueco: toda ruta declarada en TODAS_LAS_SUPERFICIES existe en disco", () => {
    // D-133-J3/T-133-04: un allowlist con una ruta muerta es un guard ciego que sale
    // verde. leerSuperficie lanza ante la primera ausencia — si esto pasa, las 63+
    // rutas declaradas resolvieron a un archivo real.
    expect(TODAS_LAS_SUPERFICIES.length).toBeGreaterThanOrEqual(63);
    for (const rel of TODAS_LAS_SUPERFICIES) {
      expect(() => leerSuperficie(rel), `ruta declarada inexistente: ${rel}`).not.toThrow();
    }
  });

  it("(1h) G3 control positivo: leerSuperficie con una ruta real devuelve contenido", () => {
    const rel = TODAS_LAS_SUPERFICIES[0];
    expect(leerSuperficie(rel).length).toBeGreaterThan(0);
  });

  it("(1i) G3 control positivo apareado: leerSuperficie con la MISMA ruta + '.noexiste' LANZA", () => {
    // Único par que difiere en UNA variable (la existencia del archivo): la ruta real
    // (1h) no lanza, la misma ruta con sufijo inexistente sí. Aísla la causa.
    const rel = TODAS_LAS_SUPERFICIES[0];
    expect(() => leerSuperficie(`${rel}.noexiste`)).toThrow(/guard ciego/);
  });
