...
--
-- ████ LOCKDOWN-01: CREAR ROL web_reader — PASO 1 de 3 — NO REVOCA NADA ████
--
...
--   * Concede un set ENUMERADO y ESTRICTO de privilegios a web_reader:
--       - SELECT en exactamente las 26 tablas public-read (auditadas en
--         _FACTS-live-prod.md, 2026-06-24). Una sentencia por tabla.
--       - EXECUTE en exactamente los 15 RPCs curados (firmas exactas de
--         RESEARCH §1). Una sentencia por RPC. NO incluye resolver_entidad.
...
--   * Tras aplicar: anon y web_reader leen idéntico (ventana segura).
--
-- POR QUÉ ENUMERAR Y NO ON ALL:
--   GRANT SELECT ON ALL TABLES / GRANT EXECUTE ON ALL ROUTINES concedería
--   acceso a vistas pgTAP (pg_all_foreign_keys, tap_funky — relkind=v, sin RLS
--   → bypass PII) y a resolver_entidad (anon=false en PROD). El set enumerado
--   es un subconjunto estricto de anon y satisface gate 3 (PISO PII).
--   Con el set enumerado, resolver_entidad NUNCA se concede → no hay nada que
--   revocar. Los RPCs secdef=f (match_proyectos, buscar_citaciones, etc.) usan
--   pgvector vía PUBLIC (=X/supabase_admin) → EXECUTE en esos operadores ya lo
--   tiene web_reader por PUBLIC y no requiere on all routines.
--
...
-- Guarda pg_auth_members para idempotencia limpia (re-run sin error).
do $$
begin
  if not exists (
    select 1 from pg_auth_members
    where member = (select oid from pg_roles where rolname = 'authenticator')
      and roleid = (select oid from pg_roles where rolname = 'web_reader')
  ) then
    execute 'grant web_reader to authenticator';
  end if;
end;
$$;
...
-- ── 4. Grants SELECT ENUMERADOS — exactamente las 26 tablas public-read ───────
-- Inventario auditado: _FACTS-live-prod.md §"26 RLS SELECT policies" (2026-06-24).
-- Una sentencia por tabla. NO `grant select on all tables` (viola gate 3).
grant select on public.aporte to web_reader;
grant select on public.aportes_ingesta_estado to web_reader;
grant select on public.citacion to web_reader;
grant select on public.citacion_invitado to web_reader;
grant select on public.citacion_punto to web_reader;
grant select on public.contrato to web_reader;
grant select on public.contratos_ingesta_estado to web_reader;
grant select on public.declaracion to web_reader;
grant select on public.declaracion_accion_derecho to web_reader;
grant select on public.declaracion_actividad to web_reader;
grant select on public.declaracion_bien_inmueble to web_reader;
grant select on public.declaracion_bien_mueble to web_reader;
grant select on public.declaracion_pasivo to web_reader;
grant select on public.declaracion_valor to web_reader;
grant select on public.lobby_audiencia to web_reader;
grant select on public.lobby_ingesta_estado to web_reader;
grant select on public.probidad_ingesta_estado to web_reader;
grant select on public.proyecto to web_reader;
grant select on public.proyecto_embedding to web_reader;
grant select on public.proyecto_ficha to web_reader;
grant select on public.sector to web_reader;
grant select on public.sesion_sala to web_reader;
grant select on public.sesion_tabla_item to web_reader;
grant select on public.tramitacion_evento to web_reader;
grant select on public.votacion to web_reader;
grant select on public.voto to web_reader;

-- ── 5. Grants EXECUTE ENUMERADOS — exactamente los 15 RPCs curados ───────────
-- Firmas exactas de RESEARCH §1 y _FACTS-live-prod.md §"15 curated public RPCs".
-- Una sentencia por RPC. NO `grant execute on all routines` (viola gate 3).
-- NO incluye resolver_entidad (anon=false en PROD; con la forma enumerada,
-- resolver_entidad nunca se concede → no hay nada que revocar).
grant execute on function public.agregado_por_contraparte(p_id text) to web_reader;
grant execute on function public.aportes_de_parlamentario(p_id text) to web_reader;
grant execute on function public.bienes_de_parlamentario(p_id text) to web_reader;
grant execute on function public.buscar_citaciones(q text, limite integer, p_camara text) to web_reader;
grant execute on function public.comparar_declaraciones(p_id text, fechas date[]) to web_reader;
grant execute on function public.contratos_de_parlamentario(p_id text) to web_reader;
grant execute on function public.cruces_de_parlamentario(p_id text) to web_reader;
grant execute on function public.declaraciones_de_parlamentario(p_id text) to web_reader;
grant execute on function public.lobby_de_parlamentario(p_id text) to web_reader;
grant execute on function public.match_proyectos(query_embedding vector, match_count integer, match_threshold double precision, exclude_boletin text) to web_reader;
grant execute on function public.parlamentario_publico(p_id text) to web_reader;
grant execute on function public.parlamentarios_publico() to web_reader;
grant execute on function public.rebeldias_de_parlamentario(p_id text) to web_reader;
grant execute on function public.subgrafo_red(p_id text, p_depth integer, p_tipos text[], p_desde timestamptz, p_hasta timestamptz) to web_reader;
grant execute on function public.votos_de_parlamentario(p_id text, p_limit integer, p_offset integer) to web_reader;

-- ── 6. Políticas RLS paralelas para web_reader (las 26 tablas public-read) ────
--
...
-- Tablas PII (deny-by-default) NO reciben policy → web_reader sigue leyendo
...
-- ── FIN LOCKDOWN-01 ───────────────────────────────────────────────────────────
-- web_reader ahora tiene un set enumerado exacto (26 tablas SELECT, 15 RPCs EXECUTE,
-- 26 policies _wr). anon conserva todos sus grants. La API pública sigue intacta.
-- Ventana segura: ambos roles leen idéntico hasta que se aplique LOCKDOWN-02 (0044).
