/**
 * GUARD-QUE-MUERDE de paridad-extracción (BENCH-02) — vitest estático, sin red.
 *
 * Tres invariantes sobre el golden congelado:
 *   1. exemplar-pool ∩ eval-pool = ∅ (anti-leakage: los few-shot NUNCA son casos de eval).
 *   2. ningún caso lleva un RUT (NO-PII por construcción, T-106-07).
 *   3. el sha256 del `casos.json` vivo coincide con el marcador `casos.freeze.json`.
 * Más los meta-tests que prueban que los guards MUERDEN (no son teatro).
 */
...
describe("extracción disjunción — anti-leakage (∩ = ∅)", () => {
